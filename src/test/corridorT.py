...
class TestCorridorZero(unittest.TestCase):
    ...
    def testSimpleOpenClose(self):
        for noof_servers in range(1, len(ALL_SANDBOX_IRC_NETWORK_NAMES)):
            ...
            shuffle(my_list_of_all_irc_servers)
    ...
    def testSimpleOpenWriteReadClose(self):
        for noof_servers in range(1, len(ALL_SANDBOX_IRC_NETWORK_NAMES)):
            my_list_of_all_irc_servers = ALL_SANDBOX_IRC_NETWORK_NAMES[:noof_servers]  # ALL_REALWORLD_IRC_NETWORK_NAMES
            the_room = '#room' + generate_random_alphanumeric_string(5)
            alice_nick = 'alice%d' % randint(111, 999)
