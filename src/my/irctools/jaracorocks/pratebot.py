    ...
    def __my_main_loop(self):
        ...
        while True:
            if self.should_we_quit:
                pass  # print("%s %-20s: %-10s: Quitting before we could finish handshakings." % (s_now(), self.irc_server, self.nickname))
            elif self.paused:
                sleep(A_TICK)
        ...
        while True:
            try:
                ...
                if msg.startswith(_REQUEST_PUBLICKEY_):
                    print("%s %-20s: %-10s: request for my pubkey    from %s" % (s_now(), self.irc_server, self.nickname, sender))
                    ...
                    print("%s %-20s: %-10s: reciprocate my pubkey      to %s" % (s_now(), self.irc_server, self.nickname, sender))
                    ...
                    print("%s %-20s: %-10s: request for my fernetkey from %s" % (s_now(), self.irc_server, self.nickname, sender))
                    if self.homies[sender].pubkey is None:
                        print("%s %-20s: %-10s: I'm being asked for my fernet key, but I don't have %s's public key yet. I'll ask for it now." % (s_now(), self.irc_server, self.nickname, sender))
                        super().put(sender, "%s%s" % (_REQUEST_PUBLICKEY_, squeeze_da_keez(self.rsa_key.public_key())))
                    else:
#                        print("%s %-20s: %-10s: I'm about to decrypt the fernet key that %s sent me." % (s_now(), self.irc_server, self.nickname, sender))
                        ...
                        if self.homies[sender].remotely_supplied_fernetkey != dc:
                            print("%s %-20s: %-10s: saving %s's fernet key" % (s_now(), self.irc_server, self.nickname, sender))
...
#                        print("%s %-20s: %-10s: I have encrypted and sent my fernet key to %s." % (s_now(), self.irc_server, self.nickname, sender))
                    ...
                    print("%s %-20s: %-10s: reciprocate my fernetkey   to %s" % (s_now(), self.irc_server, self.nickname, sender))
                    dc = rsa_decrypt(base64.b64decode(msg[len(_TRANSMIT_FERNETKEY_):]), self.rsa_key)
                    if self.homies[sender].remotely_supplied_fernetkey != dc:
                        print("%s %-20s: %-10s: saving his new fernetkey from %s" % (s_now(), self.irc_server, self.nickname, sender))
                        self.homies[sender].remotely_supplied_fernetkey = dc
                    if self.homies[sender].fernetkey is not None:
                        super().put(sender, "%s%s" % (_REQUEST__IPADDRESS_, self.my_encrypted_ipaddr(sender)))
                        print("%s %-20s: %-10s: double-rcpg my fernetkey   to %s" % (s_now(), self.irc_server, self.nickname, sender))
                elif msg.startswith(_REQUEST__IPADDRESS_):
                    print("%s %-20s: %-10s: request for my IP addr.  from %s" % (s_now(), self.irc_server, self.nickname, sender))
                    try:
                    ...
                    print("%s %-20s: %-10s: reciprocate my IP addr.  from %s" % (s_now(), self.irc_server, self.nickname, sender))
                    try:
                        ...
                        if self.homies[sender].ipaddr != new_ipaddr:
                            self.homies[sender].ipaddr = new_ipaddr
                        print("%s %-20s: %-10s: completed all handshaking  w/ %s" % (s_now(), self.irc_server, self.nickname, sender))
                    ...
                    try:
                        ...
                        print("%s %-20s: %-10s: cannot rx ciphertxt msg  from %s: fernet key issue? => %s" % (s_now(), self.irc_server, self.nickname, sender, str(e)))
                elif msg.startswith(_TRANSMIT_PLAINTEXT_):
                    try:
                        self.plain_rx_queue.put((sender, msg[len(_TRANSMIT_PLAINTEXT_):]))
                    except ValueError:
                        print("%s %-20s: %-10s: cannot rx plaintext msg  from %s" % (s_now(), self.irc_server, self.nickname, sender))
                else:
                    print("%s %-20s: %-10s: cannot rx plaintext msg  from %s => %s" % (s_now(), self.irc_server, self.nickname, sender, msg))
    ...
    def is_this_user_validly_fingerprinted(self, user):
        ...
        try:
            ...
            print("%s %-20s: %-10s: %s is a bad nickname. Ignoring." % (s_now(), self.irc_server, self.nickname, user))
            return False
            ...
            try:
                if self.is_this_user_validly_fingerprinted(user):
                    if self.homies[user].pubkey is None:
                        print("%s %-20s: %-10s: pls gimme your pubkey ktxhbai %s" % (s_now(), self.irc_server, self.nickname, user))
                        ...
                        print("%s %-20s: %-10s: pls gimme your fernetkey kbai %s" % (s_now(), self.irc_server, self.nickname, user))
                        print("%s %-20s: %-10s: I lack %s's fernet key. Therefore, I'll ask for a copy." % (s_now(), self.irc_server, self.nickname, user))
                        ...
                        print("%s %-20s: %-10s: pls gimme your IP address bai %s" % (s_now(), self.irc_server, self.nickname, user))
                        super().put(user, "%s%s" % (_REQUEST__IPADDRESS_, self.my_encrypted_ipaddr(user)))
                    else:
                        print("%s %-20s: %-10s: Yay! I have shaken hands with %s" % (s_now(), self.irc_server, self.nickname, user))
            except (IrcBadNicknameError, IrcNicknameTooLongError):
                print("%s %-20s: %-10s: %s is a crappy username. I'll ignore it." % (s_now(), self.irc_server, self.nickname, user))
